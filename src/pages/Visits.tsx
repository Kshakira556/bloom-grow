...
const isPlanCreator = Boolean(user?.id && activePlan?.created_by === user.id);
  ...
  useEffect(() => {
    const load = async () => {
      if (!user?.id || !activePlan?.id) {
        setMediationRequest(null);
        return;
      }
      try {
        setMediationError(null);
        const req = await api.getMyMediationRequestForPlan(activePlan.id);
    ...
    <div className="mt-2 p-3 border rounded-2xl bg-card w-full md:w-auto">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        ...
        {mediationRequest?.status === "pending" ? (
          ...
          <Button
            size="sm"
            disabled={mediationLoading || !user?.id || !activePlan?.id}
            onClick={async () => {
              if (!activePlan?.id) return;
              try {
                setMediationLoading(true);
                ...
                const req = await api.createMyMediationRequestForPlan(activePlan.id, {
