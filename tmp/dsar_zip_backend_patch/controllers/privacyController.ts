...
export const getMyDataBundleController = async (ctx: any) => {
  try {
    const user = ctx.state.user;
    if (!user?.id) {
      ctx.response.status = 401;
    ...
    // Reuse the same queries as JSON export (bounded).
    const userRes = await db.queryObject`
      SELECT id, full_name, email, role, phone, account_type, subscription_status, created_at, updated_at
      FROM users
      WHERE id = ${user.id}
    ...
    const childrenRes = await db.queryObject`
      SELECT DISTINCT c.id, c.first_name, c.last_name, c.birth_date
      FROM children c
      WHERE EXISTS (SELECT 1 FROM user_children uc WHERE uc.child_id = c.id AND uc.user_id = ${user.id})
      OR EXISTS (
        SELECT 1
        FROM plan_children pc
        JOIN plan_participants pp ON pp.plan_id = pc.plan_id
    ...
    const files: Record<string, Uint8Array> = {
      "README.txt": new TextEncoder().encode(
        `CUB data export (bundle)\nExported at: ${exportedAt}\n\nThis bundle contains JSON files with your account and plan-scoped records. Some fields may be redacted if a plan is pending destruction or has been redacted.\n`
      ),
      "profile.json": new TextEncoder().encode(JSON.stringify(userRes.rows[0] ?? null, null, 2)),
      "plans.json": new TextEncoder().encode(JSON.stringify(plansRes.rows, null, 2)),
      "children.json": new TextEncoder().encode(JSON.stringify(childrenRes.rows, null, 2)),
      "messages.json": new TextEncoder().encode(JSON.stringify(messagesRes.rows, null, 2)),
      "vault_documents.json": new TextEncoder().encode(JSON.stringify(documents, null, 2)),
    };
    ...
    ctx.response.headers.set("Content-Disposition", `attachment; filename=\"cub-my-data-${exportedAt.slice(0, 10)}.zip\"`);
    ctx.response.body = zipped;
  } catch (err) {
    console.error("getMyDataBundleController error:", err);
    ctx.response.status = 500;
    ctx.response.body = { error: err instanceof Error ? err.message : "Failed to export bundle" };
  }
};
