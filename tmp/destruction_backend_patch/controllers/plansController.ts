import { createPlan, listPlans, getPlanWithInvites, createPlanInvite, acceptPlanInvite, CreatePlanSchema, isUserInPlan, createPlanInviteIfNotExists, getInvitesByEmail, getInviteByToken, requestPlanDestruction } from "../services/plansService.ts";
...
export const requestPlanDestructionController = async (ctx: any) => {
  try {
    const user = ctx.state.user;
    if (!user?.id) {
      ctx.response.status = 401;
    ...
    const result = await requestPlanDestruction({ planId, userId: user.id });

    try {
      await createAuditLog({
        actor_id: user.id,
        action: "plan_destruction_requested",
        target_type: "plan",
        target_id: planId,
    ...
    ctx.response.status = 200;
    ctx.response.body = { request: result };
  } catch (error: unknown) {
    ctx.response.status = 500;
    ctx.response.body = { error: error instanceof Error ? error.message : "Unknown error" };
  }
