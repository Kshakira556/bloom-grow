...
export const setMarketingOptIn = async (args: {
  userId: string;
  optIn: boolean;
}): Promise<Pick<User, "id" | "marketing_opt_in" | "marketing_opt_in_at" | "marketing_unsubscribed_at">> => {
  const now = new Date();

  const res = await db.queryObject<Pick<User, "id" | "marketing_opt_in" | "marketing_opt_in_at" | "marketing_unsubscribed_at">>`
    UPDATE users
    SET
      marketing_opt_in = ${args.optIn},
      marketing_opt_in_at = CASE WHEN ${args.optIn} THEN ${now} ELSE marketing_opt_in_at END,
      marketing_unsubscribed_at = CASE WHEN ${args.optIn} THEN NULL ELSE ${now} END
    WHERE id = ${args.userId}
    RETURNING id, marketing_opt_in, marketing_opt_in_at, marketing_unsubscribed_at;
  `;
